        ...
        <div class="content container-fluid">
                    ...
                    <div class="col-lg-8 col-md-8">
                        <div class="invoices-tabs">
                            <ul>
                                ...
                                <li><a href="{{ url('/' . ($page = 'Admin/receivables')) }}"class="active">الذمم المالية</a>
                                </li>
            ...
            <div class="card filter-card">
                <div class="card-body pb-0">
                    <form action="{{ route('receivables_date') }}" method="post">
                        {{ csrf_field() }}
                        <div class="row formtype">
                            ...
                            <div class="col-md-2">
                                <h6>اجمالي الذمم المالية خلال الفترة المحددة</h6>
                                <h2>{{ isset($all_receivables) ? $all_receivables : '---' }}</h2>
                            </div>
                        </div>
                    </form>
                                ...
                                <table id="copy-print-csv" class="table custom-table">
                                    ...
                                    <tbody>
                                        ...
                                        @forelse ($receivables as $receivable)
                                            ...
                                            <tr>
                                                <td>{{ $i }}</td>
                                                <td><span class="badge badge-soft-dark">{{ $receivable->reco_number }}
                                                </td>
                                                <td><span class="badge badge-danger">{{ $receivable->release_date }}</td>
                                                <td><span class="badge badge-danger">{{ $receivable->due_date }}</td>
                                                <td><span class="badge badge-outline-info">{{ $receivable->total }}</td>
                                                <td><span class="badge badge-outline-info">{{ $receivable->paid }}</td>
                                                <td><span class="badge badge-outline-info">{{ $receivable->remain }}</td>
                                                <td><span class="badge badge-gardient-info"><a
                                                            href="{{ route('payment.details', $receivable->id) }}">
                                                            تفاصيل </a>
                                                </td>
                                            </tr>
                                        @empty
